...
// 4. check for every letter
// 5. calculate L and S
// 6. calculate grade level and round
// 7. check conditinos and output grade
...
int sentences(string text)
{
    ...
    {
        // for every character in end_of_sentence, check if current character
        // is that character, if so, add to sentence_count
...
{
    int word_count = 0;
    // for every letter in text, if char is space
    // and if next char is not space, add 1 to word count
    for (int letter = 0, length = strlen(text); letter < length; letter++)
    {

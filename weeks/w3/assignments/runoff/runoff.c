...
bool vote(int voter, int rank, string name)
{
    ...
    // if candidate found
    //      update preferences to voter[rank] to be index of name
    //      and return true
    // else
    //      return false
...
void tabulate(void)
{
    ...
    //              add one to vote and move on to next voter
    for (int voter = 0; voter < voter_count; voter++)
    {
        for (int candidate = 0; candidate < candidate_count; candidate++)
        {
            int candidate_index = preferences[voter][candidate];
...
int find_min(void)
{
    // lowest_vote = vote_count
    // for every candidate
    //      if candidate is not eliminated
    //          if candidate.votes < lowest_vote
    //              lowest_vote = candidate.vote
    int lowest_vote = voter_count;
    for (int candidate = 0; candidate < candidate_count; candidate++)
    {
        if (!candidates[candidate].eliminated)
        {
            if (candidates[candidate].votes < lowest_vote)
            {
                lowest_vote = candidates[candidate].votes;
            }
        }
    }
    return lowest_vote;
...
{
    // for every candidate
    //      if not eliminated and if hasn't got min --> means at least one value is more than min
    //      therefore not a tie
    //          return false
    for (int candidate = 0; candidate < candidate_count; candidate++)
    {
